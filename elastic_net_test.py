...
def agent_update_1(prev_not_i_strategies, S_i, responder=False):
    utility_feedback_vectors = np.zeros(shape=(len(prev_not_i_strategies), len(S_i)))
    max_utility = np.ones(len(prev_not_i_strategies))*5

    if responder == True: # candidate
        for t, w_f in enumerate(prev_not_i_strategies):
            for j, s_c in enumerate(S_i):
                for k, w_f_supp in enumerate(S_i):
                    if w_f_supp >= s_c:
                        ...
                        utility_feedback_vectors[t,j] += w_f[k] *  w_f_supp
    else: # firm
        for t, w_c in enumerate(prev_not_i_strategies):
            for j, s_f in enumerate(S_i):
                for k, w_c_supp in enumerate(S_i):
                    if w_c_supp <= s_f:
                        utility_feedback_vectors[t,j] += w_c[k] *  (1-s_f)
    
    # set ElasticNet model with equal weight l1, l2
    model = ElasticNet(alpha=1, l1_ratio=0.5, positive=True, fit_intercept=False, max_iter=5000)
    # print(utility_feedback_vectors)
    # print(max_utility)

    model.fit(X=utility_feedback_vectors, y=max_utility)

    # normalize
    x = model.coef_

    w_vec = x / np.sum(x) if np.sum(x) > 0 else np.zeros_like(x)
    # print("w_vec: ", w_vec)

    return w_vec.tolist() # was np array...
...
if __name__ == "__main__":
    ...
    for n in range(2,N+2):
        ...
        for t in tqdm.tqdm(range(T)):
            # if t < 1:
            #     w_f_t_p_1_1 = agent_update(prev_c,S_i=S_f,alpha_i=alpha_f, M=M, t=t)
            #     w_c_t_p_1_1 = agent_update(prev_f,S_i=S_c,alpha_i=alpha_c, M=M, responder=True,t=t)
            # else:
            w_f_t_p_1_1 = agent_update_1(prev_c,S_i=S_f)
            w_c_t_p_1_1 = agent_update_1(prev_f,S_i=S_c,responder=True)
            ...
            if t < 20:
                ...
                print([round(w,9) for w in w_c_t_p_1_1])
                print(f"w_c_t_p_1_1 support: {get_support([round(w,9) for w in w_c_t_p_1_1], S_c)}")
                print("="*40)

                print([round(w,9) for w in w_f_t_p_1])
                print(f"w_f_t_p_1 support: {get_support([round(w,9) for w in w_f_t_p_1], S_f)}")
                print([round(w,9) for w in w_f_t_p_1_1])
                print(f"w_f_t_p_1_1 support: {get_support([round(w,9) for w in w_f_t_p_1_1], S_f)}")
                print("="*40)
            else:
                break
            # else:
            ...
            prev_f_1.append(w_f_t_p_1_1)
            prev_c.append(w_c_t_p_1) 
            prev_c_1.append(w_c_t_p_1_1)
