...
def run_1_round_normal_grid(A, D,T=500, M=None, reference=None, eps=1e-7):
    ...
    if reference is not None:
        ...
        ref_str = ''
    print(D)
    filepath = f'D_{D}_M_{M}_T_{T}_del_{delta}_eps_{eps:.0e}_rounds_1{ref_str}.xlsx'
...
def run_1_round_normal_bar(A, T=500, uniform_firm=True, M=None, reference=None, eps=1e-7):
    ...
    if reference is not None:
        ...
        ref_str = ''
    filepath = f'{n_rounds}_round_ne_convergence_files_normal_bar/D_{D}_M_{M}_T_{T}_del_{delta}_eps_{eps:.0e}_rounds_{n_rounds}{ref_str}.xlsx'
...
def generate_imshow_from_sheet(A, pure, n_rounds, filepath, normal=False, ne_threshold=None):
    ...
    im = ax.imshow(util_matrix, cmap='gist_earth', vmin=0, vmax=1, origin='lower')
    ...
    for f in firm_idxs:
        for w in worker_idxs:
            ax.scatter([w], [f],c="gold",s=50,marker="*")
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig(f'{fpath}/{n_rounds}_round_imshow_{fname[0:-5]}.png') 
...
if __name__ == "__main__":
    D = 30 # discretization parameter
    T = 15000 # number of time steps ~5000 needed for 1-round, ~15000 needed for 2-round
    eta = 0.5 # eta value
    delta = 0.9
    eps = 1e-7 # convergence threshold and approx NE threshold
    n_rounds = 1
    n_trials = 100
    A = [round(i/D,4) for i in range(0,D+1)] # action list for offers {1/D, ... , 1}
    
    ## specification example of where results exist to generate plots
    # filepath = "2_round_ne_convergence_files_grid/D_5_M_0.5_T_15000_del_0.5_eps_1e-07_rounds_2.xlsx"

    ## commands used to generate plots or trees for visualization of results
    ...
    ## commands used to run grid searches over all pure initial strategies
    # run_2_round_grid_search(D, T, eta, A, delta, eps)
    # run_1_round_normal_grid(A, T, eta reference=None, eps=eps)

    ## commands used to run the algorithm on specific initial strategies.
    ## Note: initial should be a tuple of lists specifying indices for initial strategies in the order of firm (proposer), worker (responder)
    # run_n_rounds_extended_form(D, T, eta, A, eps, n_rounds, initial=[(3,4),(2,1)])
    # run_1_round_normal_bar(A, T, False, M)
    # run_1_round_normal(A,T,eta,initial=(10,10),reference=(10,20))


    ## 1-round experiments 
    D = [20, 25, 30]
    T = 5000 
    etas = [0.1,0.25,0.28854,0.5,0.6,0.8]
    eps = 1e-7
    ref_points = [None,(15, 29), (5, 15),(30, 30),(20, 16)]
    for d in D:
        A = [round(i/d,4) for i in range(0,d+1)] # action list for offers {0,1/D, ... , 1}
        for eta in etas:
            for ref in ref_points:
                run_1_round_normal_grid(A, d, T, eta, reference=ref, eps=eps)
